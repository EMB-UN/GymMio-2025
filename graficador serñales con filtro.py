...
from scipy.signal import find_peaks, butter, filtfilt  # Agrega butter y filtfilt
import scipy.signal as sp  # Importar scipy.signal como sp
...
# === FILTRO DE BANDA PARA EMG ===
high = 20 / (fs / 2)
low = 450 / (fs / 2)
b, a = sp.butter(4, [high, low], btype='bandpass')
...
try:
    while True:
        ...
        if len(buffer) == N:
            ...
            datos_mean_removed = datos - np.mean(datos)
            datos_filtrados = sp.filtfilt(b, a, datos_mean_removed)
            datos_filtrados = datos_filtrados - np.mean(datos_filtrados)

            # FFT
            fft_vals = fft(datos_filtrados)
